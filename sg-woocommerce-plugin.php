...
include( dirname( __FILE__ ) . '/includes/sg-woocommerce-helper.php' );
register_activation_hook( __FILE__, array( 'WC_Shieldgate_Database_Helper', 'create_database' ) );
register_deactivation_hook( __FILE__, array( 'WC_Shieldgate_Database_Helper', 'delete_database' ) );
...
define("PG_DOMAIN", "shieldgate.mx");
define("PG_REFUND", "/v2/transaction/refund/");

// TODO: Mover la function shieldgate_woocommerce_order_refunded
// define the woocommerce_order_refunded callback
function shieldgate_woocommerce_order_refunded($order_id, $refund_id) {
  $refund = new WC_Shieldgate_Refund();
  $refund->refund($order_id);
}

// add the action
add_action( 'woocommerce_order_refunded', 'shieldgate_woocommerce_order_refunded', 10, 2 );

if (!function_exists('sg_woocommerce_plugin')) {
  function sg_woocommerce_plugin() {
    class WC_Gateway_Shieldgate extends WC_Payment_Gateway {
      public function __construct() {
        # $this->has_fields = true;
        $this->id = 'sg_woocommerce';
        $this->icon = apply_filters('woocomerce_shieldgate_icon', plugins_url('/assets/imgs/shieldgatecheck.png', __FILE__));
        $this->method_title = 'Shieldgate Plugin';
        $this->method_description = __('This module is a solution that allows WooCommerce users to easily process credit card payments.', 'sg_woocommerce');
      ...
      public function init_form_fields() {
        $this->form_fields = require( dirname( __FILE__ ) . '/includes/admin/shieldgate-settings.php' );
      }

      function admin_options() {
        $logo = plugins_url('/assets/imgs/shieldgate.png', __FILE__);
        ?>
        <p>
          <img style='width: 30%;position: relative;display: inherit;'src='<?php echo $logo;?>'>
        </p>
        <h2><?php _e('Shieldgate Gateway','sg_woocommerce'); ?></h2>
      ...
      function receipt_page($order) {
        echo $this->generate_shieldgate_form($order);
      }
      ...
      public function generate_shieldgate_form($orderId) {
        $webhook_p = plugins_url('/includes/sg-woocommerce-webhook.php', __FILE__);
        $css = plugins_url('/assets/css/styles.css', __FILE__);
        $checkout = plugins_url('/assets/js/shieldgate_checkout.js', __FILE__);
        $orderData = $this->get_params_post($orderId);
        $orderDataJSON = json_encode($orderData)
        ?>
          <link rel="stylesheet" type="text/css" href="<?php echo $css; ?>">

          <div id="mensajeSucccess" class="hide"> <p class="alert alert-success" ><?php _e('Your payment has been made successfully. Thank you for your purchase.', 'sg_woocommerce'); ?></p> </div>
          <div id="mensajeFailed" class="hide"> <p class="alert alert-warning"><?php _e('An error occurred while processing your payment and could not be made. Try another Credit Card.', 'sg_woocommerce'); ?></p> </div>
          ...
          <button class="js-payment-checkout"><?php _e('Purchase', 'sg_woocommerce'); ?></button>

          <div id="orderDataJSON" class="hide">
            <?php echo $orderDataJSON; ?>
          </div>

          <script id="woocommerce_checkout_sg" webhook_p="<?php echo $webhook_p; ?>"
