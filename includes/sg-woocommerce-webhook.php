...
$shieldgate_message = $requestBodyJs["transaction"]['message'];
$shieldgateStoken = $requestBodyJs["transaction"]['stoken'];
...
if ($time_difference > 3 && !$shieldgateStoken) {
...
$detailPayment = array(
  ...
  10 => "System error",
  11 => "Shieldgate fraud",
  12 => "Shieldgate blacklist",
  13 => "Time tolerance",
  14 => "Expired by Shieldgate",
...
if ($shieldgateStoken) {
  $webhookObj = new WC_Gateway_Shieldgate();
  ...
  if ($stoken != $shieldgateStoken) {
    header("HTTP/1.0 203 token error");
      ...
      $order->add_order_note( __('Your payment was cancelled. Transaction Code: ', 'sg_woocommerce') . $transaction_id . __(' the reason is chargeback. ', 'sg_woocommerce'));
...
if (!in_array($statusOrder, ['completed', 'cancelled', 'refunded'])) {
    $description = __("Shieldgate Response: Status: ", "sg_woocommerce") . $status_detail .
                   __(" | Status_detail: ", "sg_woocommerce") . $detailPayment[$status_detail] .
    ...
    if ($status == 'success') {
      ...
      $order->add_order_note( __('Your payment has been made successfully. Transaction Code: ', 'sg_woocommerce') . $transaction_id . __(' and its Authorization Code is: ', 'sg_woocommerce') . $authorization_code);

    } elseif ($status == 'failure' || $status == 'pending') {
      $comments = __("Payment Failed", "sg_woocommerce");
      $order->update_status('failed');
      $order->add_order_note( __('Your payment has failed. Transaction Code: ', 'sg_woocommerce') . $transaction_id . __(' the reason is: ', 'sg_woocommerce') . $shieldgate_message);

    } else {
      $comments = __("Failed Payment", "sg_woocommerce");
      $order->add_order_note( __('The payment fails.: ', 'sg_woocommerce') );
    }
}

WC_Shieldgate_Database_Helper::insert_data($status, $comments, $description, $dev_reference, $transaction_id);
